...
for i in range(0, 3):
    lst_1.append([])
    for j in range(0, 3):
        lst_1[i].append(x*x)  # Բացատները չենք մոռանում
        x += 1
        print(lst_1[i][j], end=" ")
    print("\n")
...
x = len(nonsense)
for i in range(x):
    if nonsense[i] == 'b':  # Մեծատառ b-երն էլ պետք է հաշվենք
        counter += 1
# 5. Write a program that will print the factorial of n.
# Գրել ծրագիր, որը կտպի n թվի ֆակտորիալը։
fact = 1
n = int(input())  # Ցանկալի է input-ը միշտ ինչ-որ նամակ գրի, որ իմանանք մեզնից ինչ է սպասվում
if n == 0:
    fact = 1
else:
